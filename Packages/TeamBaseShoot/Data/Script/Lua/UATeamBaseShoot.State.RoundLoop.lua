...
--[[--------------------------------------------------------------------------
--
-- File:            UATeamBaseShoot.State.RoundLoop.lua
...
require "UATeamBaseShoot.Ui.RoundLoop"

--[[ Class -----------------------------------------------------------------]]

UATeamBaseShoot.State.RoundLoop = UTClass(UTActivity.State.RoundLoop)

-- defaults ------------------------------------------------------------------

UATeamBaseShoot.State.RoundLoop.uiClass = UATeamBaseShoot.Ui.RoundLoop

-- __ctor --------------------------------------------------------------------

function UATeamBaseShoot.State.RoundLoop:__ctor(activity, ...)
...
function UATeamBaseShoot.State.RoundLoop:Begin()
	...
	if (activity.advancedsettings.bonusmin > 0) then
		if (activity.advancedsettings.bonusmax > 0) then
			self.bonusnumber = math.random (activity.advancedsettings.bonusmin, activity.advancedsettings.bonusmax) * 1000000
		else
			self.bonusnumber = activity.advancedsettings.bonusmin * 1000000
		end
		activity.gameplayDatabonus = { 0x00, 0x01 }
		activity.gameplayData = { 0x00, 0x00 }
	end
	self.timeinterval = activity.timer

end

-- Message received from device  -----------------------------------------------------------------------

function UATeamBaseShoot.State.RoundLoop:OnDispatchMessage(device, addressee, command, arg, proxy)

	-- base

	UTActivity.State.RoundLoop.OnDispatchMessage(self)

	if (0xc3 == command) then
	
		for i, prox in ipairs(engine.libraries.usb.proxies) do

			-- ubiconnect hit

			local player = activity.players[arg[2] - 1]
			if (player and player.team.index ~= i and prox == proxy) then
				player.data.heap.hit = player.data.heap.hit + 1
				player.data.heap.score = player.data.heap.score + 1
				player.team.data.heap.score = player.team.data.heap.score + 1
				player.data.heap.hitLost = 0
				activity.uiAFP:PushLine(player.profile.name .. " " .. (l("ingame0" .. 67 + i)), UIComponent.colors[player.profile.teamColor], "base:texture/Ui/Icons/16x/Hit.tga")
			end
		end
...
function UATeamBaseShoot.State.RoundLoop:Update()

	UTActivity.State.RoundLoop.Update(self)

	if (activity.advancedsettings.bonusmin > 0) then
		local curtime = self.timeinterval - self.bonusnumber
		if (curtime >= activity.timer) then
			local num = math.random (#activity.match.players)
			player = activity.match.players[num]
			player.bonus = true
			if (activity.advancedsettings.bonusmax > 0) then
				self.bonusnumber = math.random (activity.advancedsettings.bonusmin, activity.advancedsettings.bonusmax) * 1000000
			end
		...
		if (self.timeinterval - 1000000 >= activity.timer and player.bonus) then
			player.bonus = false
		end
	end
