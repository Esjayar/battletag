...
require "UAKingoftheHill.Ui.RoundLoop"

--[[ Class -----------------------------------------------------------------]]

UAKingoftheHill.State.RoundLoop = UTClass(UTActivity.State.RoundLoop)

-- defaults ------------------------------------------------------------------

UAKingoftheHill.State.RoundLoop.uiClass = UAKingoftheHill.Ui.RoundLoop

-- __ctor --------------------------------------------------------------------

function UAKingoftheHill.State.RoundLoop:__ctor(activity, ...)
...
function UAKingoftheHill.State.RoundLoop:Begin()
	...
	activity.gameplayData = { }
	game.gameplayData = { 0x00, 0x00 }
	activity.controllingteam = nil
...
function UAKingoftheHill.State.RoundLoop:OnDispatchMessage(device, addressee, command, arg)
	...
	if (0x95 == command) then
		...
		if (device.owner and not device.rejoin) then
					...
					if (shooterDevice and shooterDevice.owner and shooterDevice.owner.primary ~= device.owner and device.owner.primary ~= shooterDevice.owner) then
						...
						if (0 == lifePoints and curIndex == 0) then
							...
							if (shooter.team ~= device.owner.team) then
								if (activity.settings.hitscoring == 1) then
									...
									shooter.team.data.heap.scorebackup = shooter.team.data.heap.scorebackup + game.settings.ActivitySettings.teampoints + game.settings.ActivitySettings.playerpoints
								end
							...
							device.owner.data.heap.isDead = 2
							device.owner.team.data.heap.nbHillPlayerAlive = device.owner.team.data.heap.nbHillPlayerAlive - 1
							device.owner.data.heap.lives = device.owner.data.heap.lives - 1
							if (activity.settings.respawntimer > 0 and activity.settings.respawnmode == 0) then
								device.owner.data.heap.playerouttimer = activity.timer
							end
							...
							if (shooter.team ~= device.owner.team) then
								if (activity.settings.hitscoring == 1) then
									...
									shooter.team.data.heap.scorebackup = shooter.team.data.heap.scorebackup + 1
								end
			...
			if (device.owner.data.heap.lifePoints < lifePoints) then
				...
				if (0 < device.owner.data.heap.isDead or game.settings.addons.medkitPack == 0) then
			...
			local hillscanned = (arg[10] * 256) + arg[11]
			
			if (device.owner.gameplayData[2] == 2) then
				device.owner.gameplayData[2] = 0
			end
			if (activity.controllingteam ~= device.owner.team and (activity.settings.respawnmode == 0 and activity.settings.numberOfTeams >= 4 and hillscanned == 6 or activity.settings.numberOfTeams < 4 and hillscanned == activity.settings.numberOfTeams or activity.settings.respawnmode == 1 and hillscanned == 1)) then
				activity.uiAFP:PushLine(device.owner.team.profile.name .. " " .. l"ingame054", UIComponent.colors[device.owner.team.profile.teamColor])
				device.owner.team.data.heap.hillpointssaved = device.owner.team.data.heap.hillpointssaved + device.owner.team.data.heap.hillpointsearned
				device.owner.team.data.heap.hillpointsearned = 0
				device.owner.gameplayData[2] = 2
				device.owner.team.data.heap.hilltimerstart = activity.timer/1000000 - activity.settings.hilldelay
				activity.controllingteam = device.owner.team
				game.gameMaster:RegisterSound({ paths = {"base:audio/gamemaster/DLG_GM_KING_OF_THE_HILL_01.wav"}})
				--DLG_GM_GLOBAL_122.wav
			end
			if (activity.controllingteam == device.owner.team) then
				if (device.owner.team.data.heap.nbHillPlayerAlive == 0) then
					activity.uiAFP:PushLine(device.owner.team.profile.name .. " " .. l"ingame055", UIComponent.colors[device.owner.team.profile.teamColor])
				end
				local pointsadded = math.floor(math.floor((device.owner.team.data.heap.hilltimerstart - activity.timer/1000000) + 0.5) * (activity.settings.hillbonus + 1) / activity.settings.interval)
				if (pointsadded > 0 and device.owner.team.data.heap.nbHillPlayerAlive >= 1) then
					device.owner.team.data.heap.hillpointsearned = pointsadded
				end
			end
			device.owner.team.data.heap.score = device.owner.team.data.heap.hillpointssaved + device.owner.team.data.heap.hillpointsearned + device.owner.team.data.heap.scorebackup
			
			if (device.owner.data.heap.isDead == 2 and device.owner.data.heap.lives == 0 or device.owner.data.heap.playerouttimer >= activity.timer + (activity.settings.respawntimer + activity.settings.respawntime + 3) * 1000000) then
				-- This player is out
				...
				if (device.owner.team.data.heap.nbPlayerAlive == 0) then
					device.owner.team.data.heap.score = 0
...
function UAKingoftheHill.State.RoundLoop:Update()
