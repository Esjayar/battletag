...
UTClass.UAClassicCapture(UTActivity)

-- state dependencies

UAClassicCapture.State = {}

	require "UAClassicCapture.State.RoundLoop"

-- default

UAClassicCapture.bitmap = "base:texture/ui/loading_ClassicCapture.tga"

-- __ctor --------------------------------------------------------------------

function UAClassicCapture:__ctor(...)
    ...
    --self.iconBanner = "base:texture/ui/Ranking_Bg_ClassicCapture.tga"
    ...
    self.scoringField = {
		...
		--{"flag", "base:texture/ui/icons/32x/flag.tga", l"iconrules007"},
    ...
    self.settings = {
        ...
        [1] = { title = l"titlemen006", options = {
            ...
            [5] = { displayMode = nil, label = l"goption007", tip = l"tip040", choices = { { value = 0, displayMode = "large", text = l"oth076" }, { value = 1, displayMode = "large", text = l"oth082" }, { value = 2, displayMode = "large", text = l"oth075" } }, index = "swap", },
            [6] = { displayMode = nil, label = l"goption036", tip = l"tip153", choices = { { value = 0, displayMode = "large", text = l"oth086" }, { value = 1, displayMode = "large", text = l"oth087" } }, index = "endgamemode", },
			[7] = { label = l"Captures", tip = l"Number of flag Captures needed to end the game", choices = { { value = 1 }, { value = 2 }, { value = 3 }, { value = 5 }, { value = 7 }, }, index = "maxCapture", },
			[8] = { label = l"Flags", tip = l"Choose weather to use T-bases 3&4 or the opposing teams bases for flags. (if more than two teams are playing, this setting will default to the other teams bases)", choices = { { value = 0, displayMode = "large", text = l"oth. team" }, { value = 1, displayMode = "large", text = l"3&4" } }, index = "flagNumber" },
            },
        },
        ...
        [2] = { title = l"titlemen007", options = {
            ...
            [5] = { label = l"goption017", tip = l"tip127", choices = { { value = 3 }, { value = 5 }, { value = 7 }, { value = 9 }, { value = 12 }, { value = 15 }, { value = 0, text = "", icon = "base:texture/ui/components/uiradiobutton_infinity.tga" } }, index = "lives", },
    ...
    self.advancedsettings = {
        ...
        [2] = { title = l"titlemen028", options = {
			...
			[1] = { displayMode = nil, label = l"goption045", tip = l"tip203", choices = { { value = false, displayMode = "large", text = l"but002" }, { value = true, displayMode = "large", text = l"but001" } }, index = "classes", },
			[2] = { displayMode = nil, label = l"goption046", tip = l"tip204", choices = { { value = 1 }, { value = 2 }, { value = 3 }, { value = 4 }, { value = 5 } }, index = "sniperhealth", },
			[3] = { displayMode = nil, label = l"goption047", tip = l"tip205", choices = { { value = 1 }, { value = 2 }, { value = 3 }, { value = 4 }, { value = 5 } }, index = "sniperammo", },
    ...
    self.playground = {
        ...
        [1] = { 
            ...
            BlueArea = { priority = 20, size = 128, text = string.format(l"psexp025"), positions = { { 480, 250 }, }, },
            ...
            RF04 = { priority = 2, title = l"oth057" ..  " (" .. l"oth028" .. ")", text = string.format(l"psexp049"), positions = { { 480, 250 }, }, },            
            RF05 = { priority = 3, title = l"oth058" ..  " (" .. l"oth029" .. ")", text = string.format(l"psexp050"), positions = { { 480, 50 }, }, condition = function (self) return (2 ~= activity.settings.flagNumber + activity.settings.numberOfTeams) end  },
            RF06 = { priority = 4, title = l"oth059" ..  " (" .. l"oth030" .. ")", text = string.format(l"psexp051"), positions = { { 50, 250 }, }, condition = function (self) return (2 ~= activity.settings.flagNumber + activity.settings.numberOfTeams and 3 ~= activity.settings.numberOfTeams) end  },           
    ...
    self.states["roundloop"] = UAClassicCapture.State.RoundLoop:New(self)
...
function UAClassicCapture:__dtor()
end

-- InitEntityBakedData  ------------------------------------------------------

function UAClassicCapture:InitEntityBakedData(entity, ranking)
...
function UAClassicCapture:InitEntityHeapData(entity, ranking)
	...
	if (entity:IsKindOf(UTTeam)) then
		...
		entity.data.heap.flagstolen = false
		...
		--applying settings for handicaps
		...
		--applying settings for classes
		entity.data.heap.audio = game.settings.audio["volume:blaster"]
		entity.data.heap.beamPower = activity.settings.beamPower
		if (activity.advancedsettings.classes) then
			if (entity.class == 2) then
				entity.data.heap.lifePoints = activity.advancedsettings.sniperhealth
				entity.data.heap.ammunitions = activity.advancedsettings.sniperammo
				entity.data.heap.audio = 0
				entity.data.heap.beamPower = 5
			elseif (entity.class == 3) then
				entity.data.heap.lifePoints = activity.advancedsettings.heavyhealth
				entity.data.heap.ammunitions = activity.advancedsettings.heavyammo
				entity.data.heap.audio = 0
				entity.data.heap.beamPower = 1
			elseif (entity.class == 4) then
				entity.data.heap.lifePoints = activity.advancedsettings.stealthhealth
				entity.data.heap.ammunitions = activity.advancedsettings.stealthammo
		...
		if (activity.settings.numberOfTeams > 2) then
			entity.data.heap.flagNumber = 2
		else
			entity.data.heap.flagNumber = activity.settings.flagNumber
		end
...
function UAClassicCapture:UpdateEntityBakedData(entity, ranking)
