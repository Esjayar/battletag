...
--[[--------------------------------------------------------------------------
--
-- File:            UASoloFrag.State.RoundLoop.lua
...
require "UASoloFrag.Ui.RoundLoop"

--[[ Class -----------------------------------------------------------------]]

UASoloFrag.State.RoundLoop = UTClass(UTActivity.State.RoundLoop)

-- defaults ------------------------------------------------------------------

UASoloFrag.State.RoundLoop.uiClass = UASoloFrag.Ui.RoundLoop

-- __ctor --------------------------------------------------------------------

function UASoloFrag.State.RoundLoop:__ctor(activity, ...)
...
function UASoloFrag.State.RoundLoop:Begin()
...
function UASoloFrag.State.RoundLoop:OnDispatchMessage(device, addressee, command, arg)
	...
	if (0x95 == command) then
		...
		if (device.owner and not device.rejoin) then
				...
				while (curIndex > 0) do
					...
					if (shooterDevice and shooterDevice.owner and shooterDevice.owner.primary ~= device.owner and device.owner.primary ~= shooterDevice.owner) then
						...
						if (0 == lifePoints and curIndex == 0) then
							...
							device.owner.data.heap.hit= device.owner.data.heap.hit + 1
			...
			if (device.owner.data.heap.lifePoints < lifePoints) then
				...
				elseif (device.owner.data.heap.lifePoints > 0 and lifePoints > device.owner.data.heap.lifePoints + 1) then
...
function UASoloFrag.State.RoundLoop:Update()
