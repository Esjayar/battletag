	...
	if (0x95 == command) then
		...
		if (device.owner and not device.rejoin) then
			...
			if (not device.owner.primary and device.owner.gameplayData[2] ~= 7) then
				if (device.owner.gameplayData[2] ~= 1 and lastBaseScanned == device.owner.team.index + 1) then
					device.owner.gameplayData[2] = 1
					activity.uiAFP:PushLine(device.owner.profile.name .. " " .. l"ingame026" .. " " .. device.owner.data.heap.lifePoints .. " " .. l"ingame027", UIComponent.colors.gray, "base:texture/Ui/Icons/16x/EnergyIcon.tga")
				...
				if (lastBaseScanned == device.owner.team.index - 1 and device.owner.gameplayData[2] == 1) then
					...
					if (device.owner.data.heap.energy == 5) then
						...
						game.gameMaster:RegisterSound({ paths = {"base:audio/gamemaster/DLG_GM_RUSTLERS_0" .. 2 + device.owner.data.heap.energy .. ".wav",}, priority = 2,})
					end
				end
			end
