...
--[[--------------------------------------------------------------------------
--
-- File:            UACapture.UILeaderboardItem.lua
...
UACapture.UILeaderboardItem = UTClass(UILeaderboardItem)
...
function UACapture.UILeaderboardItem:__ctor(leaderboard, challenger)
...
function UACapture.UILeaderboardItem:__dtor()
end

-- CreateGrid --------------------------------------------------------------

function UACapture.UILeaderboardItem:BuildItem()
    ...
    if (self.challenger:IsKindOf(UTTeam)) then
        ...
        table.foreachi(self.challenger.players, function(index, player)
			...
			if (not player.primary) then
				local uiLeaderboardItem = self:AddComponent(UACapture.UILeaderboardItem:New(self.uiLeaderboard, player), "uiLeaderboardItem" .. index)
...
function UACapture.UILeaderboardItem:Draw()
	...
	if (not self.challenger.primary) then
		if (self.challenger:IsKindOf(UTTeam)) then
	        ...
	        --[[if (self.challenger.team.data.heap.flagdevice) then
		        quartz.system.drawing.loadcolor3f(unpack(UIComponent.colors.white))
		        quartz.system.drawing.loadtexture("base:texture/Ui/Icons/16x/FlagIcon" .. self.challenger.team.index .. ".tga")
		        quartz.system.drawing.drawtexture(-128, offset - 44, 0, offset - 44 + 128)
	        end]]
			...
			if (self.challenger.team.data.heap.flagdevice == self.challenger.rfGunDevice) then
                quartz.system.drawing.loadcolor3f(unpack(UIComponent.colors[self.challenger.team.profile.teamColor]))
...
function UACapture.UILeaderboardItem:OnDataChanged(_entity, _key, _value)
...
function UACapture.UILeaderboardItem:RemoveDataChangedEvents()
...
function UACapture.UILeaderboardItem:Sort(init)
