...
--[[--------------------------------------------------------------------------
--
-- File:            UACapture.State.RoundLoop.lua
...
require "UACapture.Ui.RoundLoop"

--[[ Class -----------------------------------------------------------------]]

UACapture.State.RoundLoop = UTClass(UTActivity.State.RoundLoop)

-- defaults ------------------------------------------------------------------

UACapture.State.RoundLoop.uiClass = UACapture.Ui.RoundLoop

-- __ctor --------------------------------------------------------------------

function UACapture.State.RoundLoop:__ctor(activity, ...)
...
function UACapture.State.RoundLoop:Begin()
...
function UACapture.State.RoundLoop:OnDispatchMessage(device, addressee, command, arg)
	...
	if (0x95 == command) then
		...
		if (device.owner and not device.rejoin) then
			...
			local energy = 0
			if (activity.settings.energymode == 1 and device.owner.team.data.heap.flagdevice == device) then
            ...
            if (not device.owner.team.data.heap.flagdevice and not device.owner.primary and device.owner.gameplayData[2] ~= 7) then
                device.owner.gameplayData[4] = activity.flagbase[device.owner.team.index]
            elseif (device.owner.gameplayData[2] == 7) then
                device.owner.gameplayData[4] = 9
            elseif (device.owner.team.data.heap.flagdevice ~= device) then
                device.owner.gameplayData[4] = 10
            end
            if (device.owner.team.data.heap.lastflagdevice) then
                device.owner.gameplayData[4] = 9
                if (device.owner.team.data.heap.lastflagdevice == device) then
                    device.owner.team.data.heap.lastflagdevice = false
                end
            end
            if (device.owner.team.data.heap.flagdevice) then
                if (device.owner.team.data.heap.flagdevice.timedout or device.owner.team.data.heap.flagdevice == device and device.owner.gameplayData[2] == 7) then
                    activity.uiAFP:PushLine(device.owner.team.data.heap.flagdevice.owner.profile.name .. " " .. l"ingame065", device.owner.team.data.heap.flagdevice.owner.team.profiles[device.owner.team.data.heap.flagdevice.owner.team.index].teamcolor, "base:texture/Ui/Icons/16x/FlagIcon" .. device.owner.team.data.heap.flagdevice.owner.team.index .. ".tga")
                    device.owner.team.data.heap.flagdevice = false
                    device.owner.team.data.heap.lastflagdevice = device
                elseif (device.owner.team.data.heap.flagdevice == device) then
                    device.owner.gameplayData[4] = device.owner.team.index - 1
                end
            end
			if (lastBaseScanned == device.owner.team.index + 1 and not device.owner.team.data.heap.flagdevice and device.owner.gameplayData[4] ~= 10 and device.owner.gameplayData[2] ~= 7) then
                device.owner.team.data.heap.flagdevice = device
                device.owner.team.data.heap.lastflagdevice = false
                device.owner.gameplayData[4] = device.owner.team.index - 1
			...
			if (lastBaseScanned == device.owner.team.index - 1 and device.owner.team.data.heap.flagdevice == device) then
				-- scoring
                device.owner.team.data.heap.flagdevice = false
			...
			while (curIndex > 0 ) do
				...
				local shooter = activity.players[arg[math.min(18, curIndex + 13)] - 1]
...
function UACapture.State.RoundLoop:Update()
