					...
					if (shooterDevice) then
						...
						if (shooter) then
							...
							if (0 == device.owner.data.heap.lifePoints) then
								...
								activity.uiAFP:PushLine(shooter.profile.name .. " "  .. " has killed " .. " " .. device.owner.profile.name, UIComponent.colors.gray, "base:texture/Ui/Icons/16x/Death.tga")
								...
								if (shooter.data.heap.lastPlayerShooted == device.owner) then
									...
									if (shooter.data.heap.nbHitLastPlayerShooted == 3) then
										...
										game.gameMaster:RegisterSound({ paths = {"base:audio/gamemaster/DLG_GM_GLOBAL_26.wav",}, priority = 2,})	
