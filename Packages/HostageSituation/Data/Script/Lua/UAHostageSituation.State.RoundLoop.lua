...
--[[--------------------------------------------------------------------------
--
-- File:            UAHostageSituation.State.RoundLoop.lua
...
require "UAHostageSituation.Ui.RoundLoop"

--[[ Class -----------------------------------------------------------------]]

UAHostageSituation.State.RoundLoop = UTClass(UTActivity.State.RoundLoop)

-- defaults ------------------------------------------------------------------

UAHostageSituation.State.RoundLoop.uiClass = UAHostageSituation.Ui.RoundLoop

-- __ctor --------------------------------------------------------------------

function UAHostageSituation.State.RoundLoop:__ctor(activity, ...)
...
function UAHostageSituation.State.RoundLoop:Begin()
	...
	activity.controllingteam = activity.match.challengers[activity.settings.defendingteam]
	activity.controllingteam.data.heap.score = 1
	activity.controllingplayer = nil

end

-- Message received from device  -----------------------------------------------------------------------

function UAHostageSituation.State.RoundLoop:OnDispatchMessage(device, addressee, command, arg)
	...
	if (0x95 == command) then
		...
		elseif (device.owner.data.heap.guntime + 5000000 < quartz.system.time.gettimemicroseconds() and game.settings.GameSettings.vestdisconnect == 0 and not device.timedout) then
			if (device.owner.gameplayData[2] == 1) then
				...
				activity.uiAFP:PushLine(device.owner.profile.name .. " " .. l"ingame066", device.owner.team.profiles[device.owner.team.index].teamcolor, "base:texture/Ui/Icons/16x/FlagIcon" .. device.owner.team.index .. ".tga")
			end
		...
		if (device.owner and not device.rejoin) then
			...
			while (curIndex > 0) do
				...
				if (shooterDevice and shooterDevice.owner and shooterDevice.owner.primary ~= device.owner and device.owner.primary ~= shooterDevice.owner) then
					...
					if (0 == lifePoints and curIndex == 0) then
						...
						if (activity.settings.endgamemode == 1 and device.owner.team.data.heap.nbteamPlayerAlive == 0) then
			...
			if (lastBaseScanned == 2 and not device.owner.team.data.heap.flagstolen and device.owner.team ~= activity.controllingteam and not device.owner.primary) then
				...
				if (device.owner.team.index == activity.settings.defendingteam) then
					--game.gameMaster:RegisterSound({ paths = {"base:audio/gamemaster/DLG_GM_HOSTAGE_SITUATION_02.wav"}})
				else
					game.gameMaster:RegisterSound({ paths = {"base:audio/gamemaster/DLG_GM_HOSTAGE_SITUATION_01.wav"}})
				end
			end
			if (lastBaseScanned == device.owner.team.index - 1 and device.owner.gameplayData[2] == 1 or (activity.controllingteam.data.heap.endgametimer >= activity.timer + (activity.settings.respawnTime + 10) * 1000000 and activity.controllingteam ~= device.owner.team)) then
				device.owner.gameplayData[2] = 2
				device.owner.team.data.heap.flagstolen = false
				activity.uiAFP:PushLine(device.owner.profile.name .. " " .. l"ingame050", teamcolor, "base:texture/Ui/Icons/16x/FlagIcon" .. device.owner.team.index .. ".tga")
				device.owner.team.data.heap.score = 1
				activity.controllingteam.data.heap.score = 0
				activity.controllingplayer = device.owner
				activity.controllingteam = device.owner.team
				if (lastBaseScanned == device.owner.team.index - 1 and device.owner.gameplayData[2] == 2) then
					--game.gameMaster:RegisterSound({ paths = {"base:audio/gamemaster/DLG_GM_HOSTAGE_SITUATION_03.wav"}})
					if (activity.settings.timeleft ~= -1) then
...
function UAHostageSituation.State.RoundLoop:Update()
