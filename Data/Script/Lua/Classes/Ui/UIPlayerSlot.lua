...
function UIPlayerSlot:__ctor(...)
	...
	-- more button
		...
		self.uiButton:MoveTo( 245 + game.settings.UiSettings.nbplayerslot , -4 )
		self.uiButton.rectangle = { 245 + game.settings.UiSettings.nbplayerslot, 0, 246 + game.settings.UiSettings.nbplayerslot+ UIPlayerSlotGrid.horizontalPadding, UIPlayerSlotGrid.horizontalPadding - 1 }
...
function UIPlayerSlot:Draw()
	...
	if (self.player) then
		...
		-- player information
			...
			if (self.player.profile.icon) then
				...
				if (self.player.profile.team and self.player.profile.team > 0) then
					...
					rectangle = { -11 + math.min( 3 * (game.settings.UiSettings.nbplayerslot - 9), 24) - math.max (math.min( (game.settings.UiSettings.nbplayerslot - 9), 12), 0) , -1, -11 + math.min( 3 * (game.settings.UiSettings.nbplayerslot - 9), 24) + math.max (math.min((game.settings.UiSettings.nbplayerslot - 9), 12), 0) + UIPlayerSlotGrid.horizontalPadding - 1, UIPlayerSlotGrid.horizontalPadding - 1 }
				...
				rectangle = { -17 + math.min( 3 * (game.settings.UiSettings.nbplayerslot - 9), 24), -6, -17 +  math.min( 3 * (game.settings.UiSettings.nbplayerslot - 9), 24) + UIPlayerSlotGrid.horizontalPadding +12, -6 + UIPlayerSlotGrid.horizontalPadding + 12 }								
			...
			if (self.player.rfGunDevice and not self.player.rfGunDevice.timedout) then
  			...
  			rectangle = { 90 - game.settings.UiSettings.nbplayerslot - UIPlayerSlotGrid.horizontalPadding, -3, 94 - game.settings.UiSettings.nbplayerslot, 3 + UIPlayerSlotGrid.horizontalPadding }
			...
			local rectangle = { 85, numb[game.settings.UiSettings.nbplayerslot] }
		...
		if (self.player.class) then
			...
			local rectangle = { 215, 1, 215 + numb[game.settings.UiSettings.nbplayerslot], 1 + numb[game.settings.UiSettings.nbplayerslot] }
			quartz.system.drawing.loadcolor3f(unpack(UIComponent.colors.white))
		...
		item = {
			...
			action = function (_self) 
				for index, proxy in ipairs(engine.libraries.usb.proxies) do
					proxy:Unregister(self.player.rfGunDevice, "user deleted")
				end
			end
		}
		menu:AddItem(item)
