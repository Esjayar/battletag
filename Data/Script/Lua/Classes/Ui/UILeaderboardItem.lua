...
function UILeaderboardItem:Draw()
	...
	if (not self.challenger.primary) then
		if (self.challenger:IsKindOf(UTTeam)) then
			...
			if ((self.challenger.rfGunDevice) and (not self.challenger.rfGunDevice.timedout)) then quartz.system.drawing.loadtexture("base:texture/ui/pictograms/64x/hud_" .. self.challenger.rfGunDevice.classId .. ".tga")
