...
function UTActivity.Ui.ManualLaunch:OnClose()
	...
	for index, proxy in ipairs(engine.libraries.usb.proxies) do
		if (proxy) then
			...
			proxy._DispatchMessage:Remove(self, UTActivity.Ui.ManualLaunch.OnDispatchMessage)
...
function UTActivity.Ui.ManualLaunch:OnOpen()
	...
	for index, proxy in ipairs(engine.libraries.usb.proxies) do
		proxy._DispatchMessage:Add(self, UTActivity.Ui.ManualLaunch.OnDispatchMessage)
	end
...
function UTActivity.Ui.ManualLaunch:Update()
	...
	if (#activity.teams == 0) then
		...
		for i, player in ipairs(activity.match.challengers) do
			...
			elseif (game.settings.ActivitySettings.preventlaunch == 1 and game.settings.GameSettings.unregister == 0 and not game.settings.TestSettings.vestoverride) then
		...
		for i = 1, activity.settings.numberOfTeams do
			...
			for i, player in ipairs(activity.match.challengers[i].players) do
				if (player.rfGunDevice and player.rfGunDevice.owner and not player.rfGunDevice.timedout) then
					numberOfPlayers = numberOfPlayers + 1
				elseif (activity.category ~= UTActivity.categories.single and game.settings.ActivitySettings.preventlaunch == 1 and game.settings.GameSettings.unregister == 0 and not game.settings.TestSettings.vestoverride) then
					self.uiButton2.enabled = false
				end
			end
