...
function UTActivity.Ui.Bytecode:Draw()
    ...
    quartz.system.drawing.drawtextjustified(progress, fontJustification, unpack(rectangle))
 	...
 	-- Blaster profile being loaded
    local rectanglex = { 40, self.uiProgress.rectangle[2] - 30 - 20 + 55, 675 - 40, self.uiProgress.rectangle[2] - 30 +55 }
    local blaster = "Loading Blaster ..  " ..  activity.states["bytecode"].message
    local fontJustificationx = quartz.system.drawing.justification.center    

    quartz.system.drawing.loadcolor3f(unpack(UIComponent.colors.darkgray))
    quartz.system.drawing.loadfont(UIComponent.fonts.header)
    quartz.system.drawing.drawtextjustified(blaster, fontJustificationx, unpack(rectanglex))
    -- text
